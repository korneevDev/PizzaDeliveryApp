...
import github.mik0war.pizzadeliveryapp.core.GetDataListDiffUtilCallback
...
interface Communication<T: UIEntity> : ObserveLiveData<List<T>>, GetList<T> {
    fun updateLivaData(dataList: List<T>)

    class Base<T: UIEntity> @Inject constructor(): Communication<T> {
